...
\begin{align*}
q(z, \beta) = q(\beta\ \mid\ \alt<2->{\gvp{\lambda}}{\lambda})
\prod_{n=1}^N \prod_{j=1}^J
q(z_{n,j}\ \mid\ \alt<2->{\lvp{\phi_{n,j}}}{\phi_{n,j}})
\end{align*}
\begin{itemize}
\item<3-> Entropy decomposes 
\end{itemize}
\item<4-> Assume $q(\cdot \ \mid\ \text{variational params})$ is in (the same) exponential family as $p$
\begin{itemize}
