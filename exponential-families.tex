...
\indent $p_\alpha(x) = \dfrac{\Gamma(\sum \alpha_i)}{\prod \Gamma(\alpha_i)} \prod x_i^{\alpha_i - 1}$
\item Categorical

\indent $p_{\mathbf{p}}(x) = \prod p_i^{1[x = i]}$
\item Gaussian

\indent $p_{\mu,\sigma^2}(x) = \dfrac{1}{\sigma \sqrt{2\pi}} \exp{\left( -\dfrac{(x-\mu)^2}{2\sigma^2}\right)}$
\end{enumerate}
\end{block}
